          ...
          <Tabs value={mode} onChange={(v) => v && navigate(`/${v}`)}>
            <Tabs.List>
              {Object.entries(AUTOMATON_MODES).map(([id, { label }]) => (
                <Tabs.Tab key={id} value={id}>{label}</Tabs.Tab>
              ))}
            </Tabs.List>
          </Tabs>
          <div style={{ width: 80 }} /> {/* balances the title for centered tabs */}
        </Group>
      </AppShell.Header>
      <AppShell.Main><Outlet /></AppShell.Main>
      <AppShell.Aside p="md"><div id="aside-portal" /></AppShell.Aside>
      <AppShell.Footer px="md"><div id="footer-portal" /></AppShell.Footer>
    </AppShell>
  )
}